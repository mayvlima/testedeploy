...
class ClienteServiceTest {
    ...
    @Test
    void listarPorNome() throws ExcecaoEsperada {
        //given
        ClienteEntity cliente = new ClienteBuilder().defaultValues();
        clienteServiceTest.salvar(cliente);
        //when
        List<ClienteEntity> clienteEncontrado  = clienteServiceTest.procurarClintePeloNome(cliente.getNome());
        // then
        assertThat(clienteEncontrado.get(0).getNome()).isEqualTo(cliente.getNome());
    }

    @Test
    void listarPorCpf() throws ExcecaoEsperada {
        //given
        ClienteEntity cliente = new ClienteBuilder().defaultValues();
        clienteServiceTest.salvar(cliente);
        //when
        Optional<ClienteEntity> clienteEncontrado  = clienteServiceTest.procurarClientePorCpf(cliente.getCpf());
        // then
        assertThat(clienteEncontrado.get().getCpf()).isEqualTo(cliente.getCpf());
    }
